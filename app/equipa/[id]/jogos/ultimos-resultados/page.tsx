...
export default async function TeamCompletedMatchesPage({ params }: PageProps) {
  ...
  const matches = await getTeamMatches(teamId, 'completed').catch(() => [])
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-zinc-950 to-black px-6 py-8 text-zinc-100 md:px-8">
      <div className="mx-auto max-w-6xl space-y-6">
        <Link href={`/equipa/${teamId}`} className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-zinc-500 hover:text-indigo-300">
        ...
        <section className="flex flex-col gap-5 rounded-3xl border border-zinc-800 bg-zinc-900/35 p-6 shadow-xl md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="truncate text-2xl font-black tracking-tight text-white">Ultimos Resultados</h1>
